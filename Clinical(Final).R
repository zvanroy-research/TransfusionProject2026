# ==============================================================================
# MASTER VOLUMETRIC PIPELINE: CLINICAL THRESHOLD MODEL (RECONCILED COHORT)
# Architecture: Time-Varying Binned Dose-Response (0 vs 1-4 vs 5+ Units)
# Cutoff Paradigm: Institutional Static Floors (3000 MFI MHC-I / 1500 MFI MHC-II)
...
# ==============================================================================
# 0. MASTER PARAMETERS & FILE PATHS
# ==============================================================================
max_x_days <- 730   # 2-Year observation window ceiling
...
df_patients_clean <- df_demo_raw %>%
  ...
  filter(sex %in% c("M", "F")) %>% # FILTER OUT UNKNOWN / UNCODED SEX (sexU)
  mutate(
...
df_master_base <- df_patients_clean %>%
  ...
  mutate(
    ...
    # Strip out pre-listing baseline draws so they don't corrupt censoring timelines
    last_draw_date_post  = if_else(last_draw_date > listing_date, last_draw_date, as.Date(NA)),
    
    # Chronological mapping hierarchy
  ...
  # Exclude strictly baseline/pre-existing positive metrics (within 14 days)
  filter(is.na(first_pos_date) | as.numeric(difftime(first_pos_date, listing_date, units="days")) > 14) %>%
  # Retain patients without post-listing draws as event-free time up to removal/freeze
...
fit_stage1c <- coxph(Surv(tstart, tstop, event) ~ vol_tier, data = df_long_binned)
print(summary(fit_stage1c))
...
fit_c_s1   <- survival::survfit(fit_stage1c, newdata = data.frame(vol_tier = factor(c("0", "1-4", "5+"), levels = c("0", "1-4", "5+"))))
...
fit_c_s2 <- survival::survfit(fit_stage2c, newdata = prof_s2)
...
ggsave("Figure_1_Clinical.pdf", final_grob, width = 8.5, height = 5.2, units = "in", device = cairo_pdf)
ggsave("Figure_1_Clinical.tiff", final_grob, width = 8.5, height = 5.2, units = "in", dpi = 300, compression = "lzw")
